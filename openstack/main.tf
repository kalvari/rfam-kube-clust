...
module "generate-inventory" {
  ...
  domain                = "${ var.use_cloudflare == true ? module.cloudflare.domain_and_subdomain : format("%s.nip.io", element(concat(module.edge.public_ip, module.master.public_ip, module.glusternode.public_ip), 0))}"
  ssh_user              = "${var.ssh_user}"
  ...
  glusternode_as_edge   = "${var.glusternode_as_edge}"
  glusternode_public_ip = "${module.glusternode.public_ip}"
  glusternode_hostnames = "${module.glusternode.hostnames}"
